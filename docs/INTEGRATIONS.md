...
- [funnelkit-automation](../inc/integrations/plugins/funnelkit-automation/class-funnelkit-automation.php) — hybrid: a React app with REAL webpack-extracted stylesheets (entry + lazy route chunks). Remaps the `--wp-admin-theme-color` accent lever (Tier A) AND overrides the `.bwf-*` literals (Tier B); the route chunks inject AFTER AdminKit so their selectors need `!important`. Themes the React Flow automation canvas (nodes/edges/handles/controls/minimap) and ships a separate always-on `css/adminbar.css` for pro's admin-bar contact-search widget. One adapter, scoped to the `autonami` screen, covers free + pro.
