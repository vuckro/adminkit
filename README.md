...
- **Custom avatars** (on by default) — adds **AdminKit Portraits (Generated)** to *Settings → Discussion → Default Avatar*, the same dropdown that ships Wavatar / Identicon / Retro / MonsterID. Pick it there to give every user a unique generated portrait on a pastel-gradient backdrop, served straight from [DiceBear](https://www.dicebear.com) with a non-PII seed. AdminKit only acts when you select its option — picking Wavatar / Mystery Person / anything else leaves Gravatar's native pipeline 100% untouched. See [`docs/EXTENDING.md`](docs/EXTENDING.md#avatars) and the disclosure in `readme.txt`.
