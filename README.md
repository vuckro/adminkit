...
- **Custom avatars** (on by default) — adds **AdminKit Portraits (Generated)** to *Settings → Discussion → Default Avatar*, the same dropdown that ships Wavatar / Identicon / Retro / MonsterID. AdminKit also silently substitutes its portraits for *Mystery Person* (the WordPress factory default), so every user gets a visibly unique generated portrait — on a pastel-gradient backdrop — instead of the same mystery silhouette. A real Gravatar always wins. Pick any other Gravatar option (Wavatar, Identicon, …) and AdminKit steps aside. Served via a hosted generator ([DiceBear](https://www.dicebear.com)) with a non-PII seed. See [`docs/EXTENDING.md`](docs/EXTENDING.md#avatars) and the disclosure in `readme.txt`.
