  ...
  KEY blog_id (blog_id)
	...
	public static function get_config( $blog_id = null ) {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT parent, slug, position, icon, url, new_tab, hidden, type, title FROM {$table} WHERE blog_id = %d ORDER BY parent ASC, position ASC", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$blog_id
			),
			ARRAY_A
		);
	...
	private static function assemble( $rows ) {
		...
		foreach ( $rows as $r ) {
			...
			if ( '' === $parent ) {
				...
				$item['url']   = isset( $r['url'] ) ? (string) $r['url'] : '';
				$item['new_tab'] = ! empty( $r['new_tab'] );
