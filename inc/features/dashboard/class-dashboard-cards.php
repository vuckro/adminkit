<?php
/**
 * Dashboard cards — the presentational half of the custom dashboard.
 *
 * Pure render + data-gathering for each card (header, glance, health, content,
 * storage, preview, AdminKit quick-access, online) plus their formatting helpers.
 * The lifecycle, per-user layout/visibility state and AJAX live in
 * AdminKit_Custom_Dashboard, which calls these static methods. Split out so each
 * file stays focused (state vs. presentation).
 *
 * @package AdminKit
 */
...
class AdminKit_Dashboard_Cards {
	...
	private static function render_actions() {
		...
		echo AdminKit_Custom_Dashboard::tools_html(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- built with escaped leaves.
		echo '</div>';
	}
