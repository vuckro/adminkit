<?php
/**
 * AdminKit icon registry — the shared Heroicons (solid, 24×24) glyph set plus the
 * SVG + CSS-mask helpers used to paint them. Single source of truth for AdminKit's
 * icons, consumed by:
 *   - AdminKit_Core_Menu_Icons (the `replace_icons` feature) — masks these over
 *     stock dashicons in the admin menu + toolbar.
 *   - AdminKit_Menu_Manager (the menu manager) — per-item icon overrides + the icon
 *     picker in the settings UI.
 *
 * Icons paint as an alpha MASK, so the visible colour is always currentColor — they
 * track the surrounding text colour in light + dark. `display_svg()` is the one
 * exception: a fill:currentColor SVG for inline rendering (the picker), where CSS
 * `color` tints the glyph directly.
 *
 * @package AdminKit
 */
...
class AdminKit_Icons {

	/**
	 * The registered icon names, in declaration order.
	 *
	 * @return string[]
	 */
	...
	/**
	 * Whether a name maps to a known icon.
	 *
	 * @param string $name
	 * @return bool
	 */
	public static function has( $name ) {
		...
		$paths = self::paths();
		return ! empty( $paths[ $name ] );
	}

	/**
	 * A named icon as a complete SVG with a SOLID fill — opaque so it works as an
	 * alpha mask (the visible colour comes from currentColor via mask()). '' if
	 * the name is unknown.
	 *
	 * @param string $name
	 * @return string
	...
	/**
	 * A named icon as a display SVG (fill:currentColor) for inline rendering — e.g.
	 * the settings-UI icon picker, where CSS `color` tints it. '' if unknown.
	 *
	 * @param string $name
	 * @return string
	 */
	public static function display_svg( $name ) {
		$paths = self::paths();
		if ( empty( $paths[ $name ] ) ) {
			return '';
		}
	...
	/**
	 * The full set as `name => display SVG`, for the icon picker UI.
	 *
	...
	public static function set() {
		...
		foreach ( array_keys( self::paths() ) as $name ) {
			$out[ $name ] = self::display_svg( $name );
		}
		return $out;
	}
	 ...
	 * @return string
	 */
	public static function mask( $svg ) {
	...
	private static function paths() {
		return array(
			...
			// dashicon variants (see AdminKit_Core_Menu_Icons::menu_icon_map()).
