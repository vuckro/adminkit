 ...
 * confirm and nudge. Run `php bin/adapter-audit.php` afterwards to check the
 * finished adapter's override debt.
 *
 * Usage:
 *   php bin/adapter-scan.php <path|glob> [<path|glob> …] [options]
 *
 ...
 *
 * Examples:
 *   php bin/adapter-scan.php ../fluentform
 *   php bin/adapter-scan.php ../woocommerce/assets/client/admin --slug=woocommerce --top=60
 *   php bin/adapter-scan.php "../fluent-crm/assets/**\/*.css" --slug=fluent-crm
 *   php bin/adapter-scan.php ../slim-seo/css --slug=slim-seo --emit
 *
 * @package AdminKit
 */
...
if ( ! $paths ) {
	fwrite( STDERR, "usage: php bin/adapter-scan.php <path|glob> [more…] [--slug=] [--scope=] [--top=N] [--rtl]\n" );
	fwrite( STDERR, "       php bin/adapter-scan.php <path|glob> [more…] --slug=NAME --emit [--force]\n" );
	exit( 2 );
}
...
$scaffold = "/* {$slug} — token bridge (generated by bin/adapter-scan.php; hand-tune). */" . $nl . $nl;
...
echo "  php bin/adapter-audit.php   (Tier A target = 0 !important)" . $nl . $nl;
exit( 0 );
...
function ak_emit( $slug, $scaffold_css, $isTierA, $force, $nl ) {
	$dir       = dirname( __DIR__ ) . '/inc/integrations/' . $slug;
	...
	echo "  -> fine-tune css/admin.css (docs/ONBOARDING-A-PLUGIN.md), then: php bin/adapter-audit.php" . $nl . $nl;
}
...
function ak_class_stub( $slug, $class, $isTierA ) {
	...
	$L[] = '<?php';
	$L[] = '/**';
	$L[] = ' * ' . $class . ' — GENERATED by bin/adapter-scan.php --emit; hand-tune.';
	$L[] = ' *';
	if ( $isTierA ) {
